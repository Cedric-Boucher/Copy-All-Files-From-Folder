...
def seconds_to_time(seconds: float) -> str:
    ...
    """
    seconds = time%60 # remainder after removing seconds above 60
    minutes = int(time//60%60) # one minute is 60 seconds, so mod the time by seconds in a minute, then take the remainder after removing minutes above 60
    hours = int(time//3600%24) # one hour is 3600 seconds, so mod the time by seconds in an hour, then take the remainder after removing hours above 24
    days = int(time//86400%365) # one day is 86400 seconds, so mod the time by seconds in a day, then take the remainder after removing days above 365
    years = int(time//31536000) # one year is 31536000 seconds, so mod the time by seconds in a year
